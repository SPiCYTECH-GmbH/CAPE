...
output_path = str(Path(data_path) / "mesh_data" / gender)
start = args_dict["start"]
end = args_dict["end"]

mesh_path = "/home/mindq/repos-ext/CAPE/data/template_mesh.obj"
reference_mesh = Mesh(filename=mesh_path)
...
for i in range(start, end):
    mesh_path = str(Path(data_path) / "mesh_data" / gender / f"{i}_t_pose_mesh.obj")
    ...
    demos = demo_full(model, args.name, args.gender, args.dataset, data_dir, datadir_root,
                    ...
                    vis=bool(args.vis_demo), smpl_model_folder=args.smpl_model_folder, mesh_path=mesh_path, results_dir=output_path)
    demos.sample_vary_clotype(index=i)